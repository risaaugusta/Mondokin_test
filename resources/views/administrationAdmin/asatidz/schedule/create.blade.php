    ...
    <div class="col-md-12">
        <div class="card shadow mb-4">
            <div class="card-header py-3">
                <h6 class="m-0 font-weight-bold text-primary">Tambah Jadwal {{ env('TEXT_TEACHER', 'Asatidz') }} {{$asatidz->name}}</h6>
            </div>
            <div class="card-body">
                <form action="{{route('administrationadmin.asatidzschedule.store')}}" method="post">
                    @csrf
                    <input type="hidden" name="asatidz_id" value="{{$asatidz->id}}">
